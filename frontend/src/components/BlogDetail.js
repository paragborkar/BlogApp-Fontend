...
const BlogDetail = () => {
  ...
  const sendRequest = async ()=>{
    const res = await axios.put(`https://mern-blogapp-backend.onrender.com/api/blog/update/${id}`,{
      title:inputs.title,
  ...
  const fetchDetails = async () =>{
    ...
    const res = await axios.get(`https://mern-blogapp-backend.onrender.com/api/blog/${id}`).catch(err=>console.log(err));
    const data = await res.data;
    return data;
  }
  useEffect(()=>{
